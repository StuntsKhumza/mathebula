...
class HTMLTemplates{
    ...
    function __construct(){}
        ...
        public function doStrReplacement($body, $mappers) {
        ...
        public function writefile($body){
            ...
            $path = "mail/generated/DrNote_" . $id.".html";
...
$templateClass = new HTMLTemplates();
$body = "";

switch ($data['q']) {
    case 'drNote':

        $body = file_get_contents('../mail/templates/DrNote.html');

        break;
    
    case 'drRefNote':

        $body = file_get_contents('../mail/templates/DrRefNote.html');
...
$updatedHtml = $templateClass->doStrReplacement($body, $data['mappers']);
$gennote = $templateClass->writefile($updatedHtml);

echo json_encode(array('status'=>200, "path"=>$gennote));
